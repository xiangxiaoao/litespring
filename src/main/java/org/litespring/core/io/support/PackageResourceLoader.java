...
public class PackageResourceLoader {
    ...
    protected Set<File> retrieveMatchingFiles(File rootDir) throws IOException {
        ...
        if (!rootDir.canRead()) {
            if (logger.isWarnEnabled()) {
                logger.warn("Cannot search for matching files underneath directory [" + rootDir.getAbsolutePath() + "] because the application is not allowed to reead the directory");
            }
        }
